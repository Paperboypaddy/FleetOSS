...
export function registerMaintenanceRoutes(app: FastifyInstance) {
  ...
  // Create
  app.post<{ Body: { deviceId?: string; name?: string; type?: string; intervalDays?: number; intervalMeters?: number; lastOdometer?: number; lastDate?: string; dueOdometer?: number; dueDate?: string; notes?: string; attributes?: Record<string, unknown> } }>('/api/maintenance', async (request, reply) => {
    try {
      const db = getDb();
